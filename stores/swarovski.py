...
def fetch_cities(db, data, logger):
    ...
    for item in q('#bfselect-city-footer option[value!=""]'):
        ...
        d['node_id'] = str(d['city_code'])
        temp = item.text.strip().upper() if item.text else ''
...
def fetch_store_list(db, data, logger):
    url = data['store_url']
...
def get_data(db, table, logger=None):
    ...
    return {
        ...
        'brand_id': brand_id, 'brandname_e': u'Swarovski', 'brandname_c': u'施华洛世奇', 'node_id': u'0',
        'store_list': store_list, 'table': table}


def get_func_chain():
    return fetch_countries, fetch_states, fetch_cities, fetch_store_list, fetch_store_details

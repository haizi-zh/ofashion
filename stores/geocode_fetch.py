...
def load_city_aggr(filename):
    """
    加载city聚合数据文件，返回idcity的替换规则数据
    :param filename:
    :return:
    """
    rules = {}
    with open(filename, 'r') as f:
        ...
        for line in f.readlines():
            line_no += 1
            data = tuple(int(val[1]) for val in re.findall(ur'(^|\t)+(\d+):', line))
            if len(data) <= 2:
                print(unicode.format(u'Something wrong in line {0}', line_no))
                continue

            target_id = data[data[0]]
            id_changed = set(filter(lambda val: val != target_id, data[1:]))
            if target_id in rules:
                map(rules[target_id].add, id_changed)
            else:
...
def merge_city_mapping(outfile, mapping_1, mapping_2):
    def insert_mapping_record(mapping, val):
        """
        将某个城市信息（格式：(id_key, set(ids_sub)）添加到mapping中。
        :param mapping:
        :param val:
        """
        id_key, ids_sub = val
        # id_target是否在mapping中存在？
        if id_key in mapping:
            map(mapping[id_key].add, ids_sub)
        elif id_key in reverse_mapping:
            while True:
                # 需要添加的id_key在旧mapping中作为id_sub出现。发生矛盾
                opt = raw_input(unicode.format(u'Conflict detected. Which one to use? Press 1 for {0}, 2 for {1}: ',
                                               reverse_mapping[id_key], id_key))
                if opt == 1:
                    id_key_prime = reverse_mapping[id_key]
                    map(mapping[id_key_prime].add, ids_sub)
                    mapping[id_key_prime].add(id_key)
                    break
                elif opt == 2:
                    id_key_prime = reverse_mapping[id_key]
                    val = mapping[id_key_prime]
                    del mapping[id_key_prime]
                    mapping[id_key] = val
                    map(mapping[id_key].add, ids_sub)
                    for val in map[id_key]:
                        reverse_mapping[val] = id_key
                    break
                else:
                    continue
        else:
            mapping[id_key] = ids_sub
            for val in ids_sub:
                reverse_mapping[val] = id_key
    ...
    reverse_mapping = {}
    if mapping_1:
        with open(mapping_1, 'r') as f:
            original_mapping = dict((int(temp[0]), set(temp[1])) for temp in json.load(f).items())

    for key in original_mapping:
        for val in original_mapping[key]:
            if val not in reverse_mapping:
                reverse_mapping[val] = key


    # 导入人工编辑的信息
    edited_mapping = load_city_aggr(mapping_2)
    for item in edited_mapping:
        insert_mapping_record(original_mapping, (item, edited_mapping[item]))

    with open(outfile, 'w') as f:
        json.dump(dict((temp[0], tuple(temp[1])) for temp in original_mapping.items()), f, ensure_ascii=False,
                  encoding='utf-8')

    return original_mapping


def detect_city_aggr(db, threshold=0, threshold2=0.2, outfile_1=None, mapping_1=None):
    """
    分别针对每个国家，查找其下的城市是否有靠得非常近的。如果同一个国家的两个城市距离很小，甚至位置相同，很可能
    它们是同一个城市的不同写法。需要对此进行聚合，归并。
    :param mapping_1: 旧的city_mapping信息，JSON格式
    :param db:
    :param threshold: 距离小于该阈值的两个城市，添加到合并候选列表中。
    :param threshold2: 两个城市，如果距离小于该阈值，则视为坐标相同（因为涉及到浮点精度误差，可能两个相同的坐标，在计算相对距离时也会得到不为零的结果。
    :param outfile_1: JSON格式的输出信息
    :param outfile_2: 输出，待人工编辑
    :return:
    """
    ...
    if mapping_1:
        with open(mapping_1, 'r') as f:
            original_mapping = dict((int(temp[0]), set(temp[1])) for temp in json.load(f, encoding='utf-8').items())
    reverse_mapping = {}
    for key in original_mapping:
        for val in original_mapping[key]:
            if val not in reverse_mapping:
    ...
    with codecs.open(outfile_1, 'w', 'utf-8') as output:
        for c in country_list:
            ...
            city_list = []
            for item in results:
                db.query(str.format('SELECT COUNT(idstores) FROM stores WHERE idcity={0}', item['idcity']))
                cnt = int(db.store_result().fetch_row()[0][0])
                if cnt > 0:
                    city_list.append((int(item['idcity']), float(item['lat']), float(item['lng'])))
            ...
            if len(identicals) > 0:
                for loc in identicals:
                    ...
                    # 如果原始mapping中包含
                    id_key = None
                    for id_sub in idcity_list:
                        if id_sub in original_mapping:
                            id_key = id_sub
                            break
                        elif id_sub in reverse_mapping:
                            id_key = reverse_mapping[id_sub]
                            break

                    if id_key:
                        ids_total = list(original_mapping[id_key].union(idcity_list))
                        if id_key in ids_total:
                            idx = ids_total.index(id_key) + 1
                        else:
                            idx = 0
                    else:
                        ids_total = list(idcity_list)
                        idx = 0

                    city_list = []
                    for idcity in ids_total:
                        db.query(str.format(
                    ...
                    output.write(unicode(idx) + u':\t\t' + msg + u'\n')
                    print(msg)
...
def geocode_query(db, id_range=None, extra_condition=None, overwrite=False, logger=logging.getLogger()):
    ...
    def gen_search_list(result):
        ...
        :param result:
        """
        country, province, city_e, city_c = (result[key] for key in ('country_e', 'province_e', 'city_e', 'city_c'))
        if result['addr_e_rev']:
            addr = result['addr_e_rev']
        elif result['addr_c_rev']:
            addr = result['addr_c_rev']
        elif result['addr_e']:
            addr = result['addr_e']
        elif result['addr_c']:
            addr = result['addr_c']
        ...
        city_e = city_e if city_e else (city_c if city_c else u'')
        country = country if country else u''
        return ','.join((addr, city_e, country)), addr
    ...
    for i in xrange(total_cnt):
        ...
        logger.info(
            unicode.format(u'{0}/{1} completed({2:.2%}): idstores={3}, brand_id={4}, brand_name={5}', i, total_cnt,
                           float(i) / total_cnt, idstores, result['brand_id'], result['brandname_e']))
...
def process_geocode_data(db, id_range=None, refine=False, overwrite=False, extra_condition=None, block_size=500,
                         db_local=None, logger=logging.getLogger()):
    """
    ...
    if not extra_condition:
        ...
        extra_condition = list(extra_condition)
    if not overwrite:

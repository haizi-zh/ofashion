...
if __name__ == "__main__":
    ...
    # 进行地址过滤
    # addr_filter.addr_filter(db, extra=(u'flag=1', u'idstores>=49655'), update=True, logger=logger)

    # common.reset_geo(db, extra=(u"update_time>'2013-09-24 17:00:00'",), logger=logger)

    # gf.geocode_query(db, overwrite=True,
    #                  extra_condition=('((brand_id between 10128 and 10266) && idstores>=366674 && flag=1 && addr_e_rev is not null && '
    #                                   '(geo_queried="fail" || geo_route is null))',),
    #                  logger=logger)
    # gf.geocode_query(db, overwrite=True,
    #                  extra_condition=('(city_e IS NULL && city_c IS NOT NULL)',),
    #                  logger=logger)

    # gf.geocode_query(db, extra_condition=('(brand_id BETWEEN 10378 AND 10388)',), overwrite=True, logger=logger)
    # gf.geocode_query(db, extra_condition=('idstores>=875462',), overwrite=True, logger=logger)
    # gf.process_geocode_data(db, id_range=None, refine=True, overwrite=True,
    #                         extra_condition=(u"update_time>'2013-09-27 00:00'",), logger=logger)
    # gf.process_geocode_data(db, None, True, overwrite=True,
    #                         extra_condition=('(brand_id IN (10360,10362))',),
    #                         logger=logger)
    ...
    # geocode_fetch.load_city_aggr(u'../data/city_mapping_round_1.txt')
    # geocode_fetch.detect_city_aggr(db, outfile_1=u'../data/test1.txt', mapping_1=u'../data/outfile.txt')
    # geocode_fetch.merge_city_mapping(u'../data/outfile.txt', u'../data/outfile.bak.txt',
    #                                  u'../data/city_mapping_round_1.txt')

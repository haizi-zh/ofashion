...
class MiumiuSpider(MFashionSpider):
        ...
        if subtitles:
            for node1 in subtitles:
                ...
                for node2 in node1.xpath('../div[@class="products_list"]/div[contains(@class,"product")]/'
                    ...
                    if tmp:
                        m2['name'] = self.reformat(cm.unicodify(tmp[0]._root.text))
    ...
    def parse_details(self, response):
        ...
        if tmp:
            metadata['color'] = [self.reformat(cm.unicodify(tmp[0]._root.text))]
        ...
        metadata['details'] = '\r'.join(
            ','.join(filter(lambda val: val, (self.reformat(cm.unicodify(val)) for val in
